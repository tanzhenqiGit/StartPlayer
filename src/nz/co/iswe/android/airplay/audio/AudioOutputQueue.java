...
public class AudioOutputQueue implements AudioClock {
	...
	private String TAG = "AudioOutputQueue";
	 ...
	 */
	private volatile boolean mIsQueueClosed = false;
...
//		audioTrack = new AudioTrack(streamType, sampleRateInHz, channelConfig, audioFormat, bufferSizeInBytes, mode); //wu3316
		mAudioTrack = new AudioTrack(streamType, sampleRateInHz, AudioFormat.CHANNEL_CONFIGURATION_STEREO, audioFormat, bufferSizeInBytes, mode);//FIXME
		...
		while ( queueThread.isAlive() && mAudioTrack.getPlayState() != AudioTrack.PLAYSTATE_PLAYING){
			Thread.yield();
		}
		*/
	...
	private class EnQueuer implements Runnable {
		...
		public void run() {
			try {
				...
				mAudioTrack.play();
				SLog.d(TAG, "Enquer run Audio Track started");				
				boolean lineMuted = true;
				boolean didWarnGap = false;
				while (!mIsQueueClosed) {
					if (!frameQueue.isEmpty()) {
						...
						SLog.d(TAG, "run gapFrames:" + gapFrames + "packetSizeFrames:" + packetSizeFrames);
						if (gapFrames < -packetSizeFrames) {
							...
							if (lineMuted) {
								SLog.d(TAG, "Audio data available, Un Mute");
								lineMuted = false;
								applyVolume();
							...
							SLog.d(TAG, "Audio data containing " + nextPlaybackSamplesLength / bytesPerFrame + " frames for playback time " + entryFrameTime + " found in queue, appending to the output line");
...
//			
			...
			final int bytesWritten = mAudioTrack.write(samplesConverted, 0, samplesConverted.length);
			
			if (bytesWritten == AudioTrack.ERROR_INVALID_OPERATION) {
				SLog.d(TAG, "Audio Track write bytesWritten == AudioTrack.ERROR_INVALID_OPERATION");
				...
				SLog.d(TAG, "Audio Track write Wrong parameters.");
				...
				SLog.d(TAG, "Audio Track write only " + bytesWritten + " bytes of sample data while trying to write " + samples.length + " bytes");
			} else{
				SLog.d(TAG, "Audio Track write" + bytesWritten + " bytes");
			}
	...
	private void setStereoVolume(float leftVolume, float rightVolume) {
		...
		SLog.d(TAG, "setStereoVolume leftVolume: " + leftVolume + " rightVolume: " + rightVolume);
	...
	private long getNowLineTime() {
		...
		if(mAudioTrack.getPlayState() == AudioTrack.PLAYSTATE_PLAYING){
			long value = mAudioTrack.getPlaybackHeadPosition();
			return value;
		}
		else{
			LOG.warning("getNowLineTime() called while mAudioTrack is not on a Playing State");
			return 0;
		}
	}
